...
export function SpadesGame({ balance, onBack, onBet, onWin, cardBackStyle }: SpadesGameProps) {
  ...
  const playCard = (cardIndex: number) => {
    ...
    aiTimer.current = setTimeout(() => playAICards(newTrick, newPlayers), 650);
  };
        ...
        <div className="pt-12 flex flex-col flex-1">
          ...
          {(gamePhase === 'dealing' || gamePhase === 'bidding' || gamePhase === 'playing') && (
            <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
              ...
              {/* SCORE BAR */}
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-white/5" style={{ background: 'rgba(0,0,0,0.6)' }}>
                <div className="flex items-center gap-3">
                  <div className="text-xs text-gray-500 uppercase">Round {round}</div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-16 h-2 rounded-full bg-black/50 overflow-hidden border border-[#43A047]/30">
                    ...
                    <span className="text-gray-600 text-xs">· {bags.you}🎒</span>
                  </div>
                </div>
                <div className="text-xs font-bold text-[#D4AF37]/60">Goal: {houseRules.targetScore}</div>
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5">
                    <span className="text-gray-600 text-xs">{bags.opponent}🎒 ·</span>
                    <span className="text-[#ef5350] text-xs font-bold">{teamScore.opponent}</span>
                    <span className="text-gray-600 text-xs">pts</span>
                    <div className="w-16 h-2 rounded-full bg-black/50 overflow-hidden border border-[#B71C1C]/30">
              ...
              >
                ...
                <div className="absolute top-3 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 z-10">
                  ...
                  <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/70 border border-[#1565C0]/40 backdrop-blur-sm">
                    ...
                    <div>
                      <div className="text-xs font-bold text-[#64b5f6]">{players[2].name}</div>
                      <div className="text-xs text-gray-400">{players[2].nilBid ? '🚫NIL' : `Bid: ${players[2].bid ?? '?'}`} · {players[2].tricks}✓</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/80 border border-[#D4AF37]/35 backdrop-blur-sm">
                  ...
                  <div>
                    <div className="text-xs font-bold text-[#D4AF37]">You</div>
                    <div className="text-xs text-gray-400">{players[0].nilBid ? '🚫NIL' : players[0].blindNilBid ? '🔮BNIL' : `Bid: ${players[0].bid ?? '?'}`} · {players[0].tricks}✓</div>
                  </div>
                </div>
                ...
                {gamePhase === 'bidding' && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none" style={{ zIndex: 25 }}>
                    <div className="pointer-events-auto bg-black/80 backdrop-blur-md rounded-2xl px-6 py-5 border border-[#D4AF37]/30 shadow-2xl"
                      style={{ animation: 'bidPop 0.35s cubic-bezier(0.34,1.56,0.64,1) both' }}>
                      <div className="text-center text-white font-bold text-lg mb-4">Your turn to bid</div>
                      <div className="grid grid-cols-7 gap-1.5 mb-2">
                        {houseRules.nilAllowed && (
                          <button onClick={() => placeBid(0, true, false)}
                            className="col-span-1 px-0 py-2.5 rounded-xl font-black text-sm bg-blue-600 hover:bg-blue-500 text-white transition-all hover:scale-110 shadow-lg">
                            Nil
                          </button>
                        )}
                        {[1, 2, 3, 4, 5, 6].map(n => (
                          <button key={n} onClick={() => placeBid(n)}
                            className="py-2.5 rounded-xl font-black text-sm bg-white hover:bg-[#D4AF37] text-gray-800 hover:text-black transition-all hover:scale-110 shadow-md">
                            {n}
                          </button>
                        ))}
                      </div>
                      <div className="grid grid-cols-7 gap-1.5">
                        {[7, 8, 9, 10, 11, 12, 13].map(n => (
                          <button key={n} onClick={() => placeBid(n)}
                            className="py-2.5 rounded-xl font-black text-sm bg-[#d0d0c0] hover:bg-[#D4AF37] text-gray-600 hover:text-black transition-all hover:scale-110 shadow-md">
                            {n}
                          </button>
                        ))}
                      </div>
                      {houseRules.blindNilAllowed && (
                        <div className="mt-2">
                          <button onClick={() => placeBid(0, false, true)}
                            className="w-full py-2 rounded-xl font-bold text-sm border-2 border-purple-500 text-purple-300 bg-purple-900/30 hover:bg-purple-800/50 transition-all hover:scale-105">
                            🔮 Blind NIL (±200)
                          </button>
                        </div>
                      )}
