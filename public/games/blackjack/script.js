    ...
    const writeResult = (kind, stake, payout, detail) => {
      ...
      if (send)
        setTimeout(() => {
          ...
          betStatusInfo[0].textContent = toDE(playerInfo.balance) + ' $Pc';
        }, 500);
    };
...
function surrender() {
  ...
  if (betZoneInfo[activeZone].scoreZone)
    betZoneInfo[activeZone].scoreZone.textContent = '0 $Pc';
  betZoneInfo[activeZone].status = false;
}
...
function evenMoney() {
  ...
  z.status = false;
  z.bet = 0;
  if (z.scoreZone) z.scoreZone.textContent = '0 $Pc';
...
function undoLastBet() {
  ...
  if (el) {
    if (newTotal > 0) {
      el.textContent = newTotal.toLocaleString('de-DE') + ' $Pc';
      el.style.opacity = 1;
    } else {
      el.textContent = '';
      el.style.opacity = 0;
...
function rebet() {
  ...
  const setUIValue = (sel, idx, val) => {
    ...
    if (els && els[idx]) {
      els[idx].textContent = val.toLocaleString('de-DE') + ' $Pc';
...
window.addEventListener('message', function(e) {
  ...
  var overlay = document.getElementById('bjBotOverlay');
  if (!overlay) {
    overlay = document.createElement('div');
    overlay.id = 'bjBotOverlay';
  ...
  e.data.bots.forEach(function(bot) {
    ...
    if (bot.currentBet > 0) {
      ...
      cardWrap.style.cssText = 'display:flex;gap:1px;margin-left:4px;';
      for (var c = 0; c < 2; c++) {
        var card = document.createElement('div');
        card.style.cssText = 'width:10px;height:14px;border-radius:1px;background:linear-gradient(135deg,#2a5c94,#1a3d66);border:0.5px solid rgba(255,255,255,0.2);';
